...
export default function DownloadButton({
  ...
  // Expect a complete API policy object
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.headerWrap}>
          ...
          {/* Title prefers API title; falls back to type */}
        ...
        <Section title="POLICY INFORMATION">
          ...
          <Row label="Policy ID" value={safePolicy.policyId ?? "-"} />
          <Row
            label="Customer ID"
            value={
              safePolicy.customerId ?? safePolicy.userId ?? userIdForPdf ?? "-"
            }
          />
          <Row
            label="Policy Type"
            value={
              safePolicy.type ? String(safePolicy.type).toUpperCase() : "-"
            }
          />
          <Row
            label="Status"
            value={safePolicy.status ?? safePolicy.policyStatus ?? "-"}
          />
        </Section>
        ...
        <Section title="COVERAGE & PREMIUM">
          <Row
            ...
            value={`Rs. ${formatINR(safePolicy.sumInsured ?? 0)}`}
          />
          <Row
            label="Premium"
            value={`Rs. ${formatINR(safePolicy.premium ?? 0)}`}
          />
        </Section>

        {/* Section: Policy Period */}
        <Section title="POLICY PERIOD">
          <Row label="Start Date" value={formatDate(safePolicy.startDate)} />
          <Row label="End Date" value={formatDate(safePolicy.endDate)} />
        </Section>

        {/* Section: Perks */}
        {Array.isArray(safePolicy.perks) && safePolicy.perks.length > 0 && (
          <Section title="PLAN PERKS">
            <View>
              {safePolicy.perks.map((perk, idx) => (
        ...
        {/* Section: Terms & Coverage Details */}
        <Section title="TERMS & COVERAGE DETAILS">
          <Text style={styles.blockLabel}>Terms</Text>
          <Text style={styles.blockText}>
            {safePolicy.terms || "Policy details not available"}
          </Text>
        </Section>

        {/* Optional: Attachments */}
        {Array.isArray(safePolicy.attachments) && (
          <Section title="ATTACHED DOCUMENTS">
            <Text style={styles.blockText}>
              {safePolicy.attachments.length > 0
                ? safePolicy.attachments
                    .map((a, i) => `${i + 1}. ${a?.name || a}`)
                    .join("\n")
                : "No attachments."}
            </Text>
          </Section>
  ...
  const resolvedFileName =
    fileName || `Policy_${safePolicy.policyId ?? "document"}.pdf`;
